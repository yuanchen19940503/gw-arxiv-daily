...
更新结果见：https://yuanchen19940503.github.io/gw-arxiv-daily/

## 中文说明

### 1. 项目简介
本项目用于每天抓取 arXiv 的 `gr-qc/new` 与 `astro-ph/new`（可自行修改），在“New submissions + Cross-lists”中检索与引力波相关的论文，并生成：
- 当天批次的 JSON 文件（永久保存）
...
- **关键词匹配**：当前匹配的关键词为 GW(s) / gravitational-wave(s) / gravitational wave(s) （可自行修改）
- **过滤 replaced**：不统计 “replaced” 条目
- **按批次日期归档**：以 arXiv 更新为准，如果当天 arXiv 不更新，则不产生新批次、不写新数据
- **静态网页展示**：
  - 当日匹配列表（默认隐藏作者，按钮一键显示/隐藏）
  - 归档链接（默认展示最近 30 次更新批次的 JSON）
  - Top 作者（累计匹配次数）
  - 支持作者筛选（如你启用了对应按钮/交互逻辑）
  - 未来我可能会做一个可视化统计，输出一下该领域每个月的论文数等等，绘制成图
---
